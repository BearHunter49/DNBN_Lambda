...
def main(event, context):
    ...
    # rek = boto3.client('rekognition')
    # responseRekog = rek.get_face_detection(JobId="1201b14bb85b9bbf37008256da524f0a33aef34b397a666158177f35648af06e", MaxResults=1)
    
    # message = event['Records'][0]['Sns']['Message']
    
    logs = boto3.client('logs')
    LOG_GROUP = 'rekognition_test'
    LOG_STREAM = 'Mytest2'
    
    res = logs.create_log_stream(logGroupName=LOG_GROUP, logStreamName=LOG_STREAM)
    
    timestamp = int(round(time.time() * 1000))
    
    response = logs.put_log_events(logGroupName=LOG_GROUP, logStreamName=LOG_STREAM, logEvents=[{
        'timestamp': timestamp,
        'message': str(event)
    }])
